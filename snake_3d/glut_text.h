...
#if (__cplusplus >= 201103L) // Verifica a versão do compilador C++
  using std::to_string; // usada a to_string do namespace std se a versão for maior ou igual ao C++11
#else
  using my::to_string;  // usada a to_string do namespace my em caso contrario
#endif

/* Converte números em std::string */
...
/* Desenha texto stroke na posição (x,y) */
